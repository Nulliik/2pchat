...
Прикладная совместимость развивается независимо от номера handshake и шифропакета.
После bootstrap capabilities передаются в существующем зашифрованном `identity_info`.
Go хранит результат на уровне соединения; JNI возвращает его копию для Kotlin/UI.
Клиент без декларации сохраняет базовый личный чат, дополнительные групповые функции
требуют подтверждённой поддержки. Переподключение сбрасывает согласование.
Матрица, модель угроз и правила следующих релизов: [PROTOCOL_COMPATIBILITY.md](PROTOCOL_COMPATIBILITY.md).
