...
from messenger.core.session import PROTOCOL_V2, PROTOCOL_V3, Session
...
def test_protocol_vectors_legacy_and_v3_handshakes_match_fixture():
    ...
    v2_session = _build_reference_session(
        identity_priv,
        prekey_priv,
        signing_key,
    ...
    assert Session._handshake_payload(v2_session).decode("utf-8") == vectors["legacy_handshake"]["payload_utf8"]
    assert Session._x3dh_payload(v3_session, "init").decode("utf-8") == vectors["x3dh_handshake"]["init_payload_utf8"]

    legacy_pub, legacy_verify, legacy_prekey = Session._parse_handshake(
        base64.b64decode(vectors["legacy_handshake"]["payload_base64"])
    )
    x3dh = Session._parse_x3dh_handshake(
        base64.b64decode(vectors["x3dh_handshake"]["init_payload_base64"])
    )

    assert base64.b64encode(bytes(legacy_pub)).decode() == vectors["identity"]["identity_public_base64"]
    assert base64.b64encode(bytes(legacy_verify)).decode() == vectors["identity"]["verify_public_base64"]
    assert base64.b64encode(bytes(legacy_prekey)).decode() == vectors["identity"]["prekey_public_base64"]
...
def test_protocol_vectors_encrypted_packet_matches_legacy_fixture(monkeypatch):
    vectors = _load_vectors()
    identity_priv = PrivateKey(bytes.fromhex(vectors["identity"]["identity_seed_hex"]))
    peer_priv = PrivateKey(bytes.fromhex(vectors["identity"]["peer_seed_hex"]))
    prekey_priv = PrivateKey(bytes.fromhex(vectors["identity"]["prekey_seed_hex"]))
    ephemeral_priv = PrivateKey(bytes.fromhex(vectors["legacy_encrypted_chat"]["ephemeral_seed_hex"]))
    nonce = bytes.fromhex(vectors["legacy_encrypted_chat"]["nonce_hex"])
    chat_object = vectors["messages"]["chat_object"]
    ...
    assert packet.hex() == vectors["legacy_encrypted_chat"]["packet_hex"]
    ...
    assert decrypted.decode("utf-8") == vectors["legacy_encrypted_chat"]["decrypted_json_utf8"]
