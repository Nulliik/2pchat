                ...
                if (PythonBridge.isInitialized) {
                    val snapshot = runCatching {
                        withContext(Dispatchers.IO) {
                            ...
                            HeroSnapshot(
                                activePeers = PythonBridge.getActivePeers().distinct().size,
                                upnpOk = PythonBridge.isUpnpMapped(),
                                trackersOk = trackers.isNotEmpty() && trackers.values.any {
                                    it.contains("announce=ok", ignoreCase = true)
                                },
                                yggOk = PythonBridge.getYggdrasilAddress().let { address ->
