...
/**
 * Ed25519 signatures remain in the canonical Python identity implementation.
 * The signing key never crosses the Chaquopy boundary.
 */
object GroupIdentitySignatures {
    const val DOMAIN = "2pchat-group-event-signature-v1"

    fun localVerificationKey(): String = PythonBridge.getLocalSigningPublicKey()

    fun sign(canonicalPayload: String): String =
        PythonBridge.signGroupPayload(canonicalPayload)

    fun verify(
        verificationKeyBase64: String,
        canonicalPayload: String,
        signatureBase64: String,
    ): Boolean = PythonBridge.verifyGroupPayload(
