...
object P2PMessageRelay {
    ...
    fun renamePeer(context: Context, oldName: String, newName: String): Boolean {
        ...
        synchronized(identityLock) {
            ...
            serviceScope.launch(Dispatchers.Main) {
                ...
                peerAvatars[oldName]?.let { av ->
                    peerAvatars[cleanNewName] = av
                    peerAvatars.remove(oldName)
                }
            }
        }
        return true
                        ...
                        if (trimmed.startsWith("{")) {
                            ...
                            when (json.optString("type")) {
                                "chat" -> {
                                    ...
                                    if (nickname != null && (sender.length > 30 || !sender.all { it.isLetterOrDigit() || it == '_' })) {
                                        sharedPrefs.edit()
                                ...
                                "onion_address_share" -> {
                                    ...
                                    if (formattedOnion != null && formattedOnion.contains(".onion", ignoreCase = true)) {
                                        ...
                                        val fingerprint = P2PPreferences.prefs(appContext)
                                            .getString("peer_fingerprint_$resolvedSender", null) ?: (if (sender.length > 30) sender else null)
                                        ...
                                        if (lastOnionShareAt[resolvedSender] == null) {
                                ...
                                "text" -> {
                                    ...
                                    persistAndDispatchIncoming(appContext, resolvedSender, rxMsg)
                                    return
                                }
                                "reply" -> {
                                    val msgId = json.optString("message_id")
            ...
            bridge.registerSessionListener(object : com.example.twopchat.bridge.BridgeSessionListener {
                override fun onSessionEstablished(peerName: String, fingerprint: String, endpoint: String, transport: String, aboutMe: String): Boolean {
                    ...
                    if (isPlaceholderPeerName(resolvedPeerName)) {
                        ...
                        shareAvatar(appContext, fingerprint, endpoint)
            ...
            try {
                ...
                if (b64 != null || myUsername.isNotEmpty() || myAboutMe.isNotEmpty()) {
                    ...
                    if (!bridge.isPeerOnline(peerName, expectedFingerprint)) {
                        log(context, "Peer $peerName is offline; skipping profile share")
                        return@launch
                    }
    ...
    fun deleteChat(context: Context, peerName: String) {
        ...
        relayScope.launch {
            try {
                getBridge(context).closePeerSession(peerName, expectedFingerprint)
            } catch (e: Exception) {
