...
// MnemonicFromSeed converts 32 bytes (256-bit entropy) into a 24-word BIP-39 mnemonic phrase.
func MnemonicFromSeed(seed []byte) (string, error) {
	if len(seed) != 32 {
		return "", fmt.Errorf("invalid seed length: expected 32 bytes, got %d", len(seed))
	}
	...
	mask := big.NewInt(2047) // 11 bits (0x7FF)
...
// SeedFromMnemonic decodes a 24-word BIP-39 mnemonic phrase back into 32-byte seed with checksum validation.
func SeedFromMnemonic(mnemonic string) ([]byte, error) {
	words := strings.Fields(strings.TrimSpace(mnemonic))
	if len(words) != 24 {
		return nil, fmt.Errorf("invalid mnemonic word count: expected 24 words, got %d", len(words))
	}

	wordMap := make(map[string]int64, len(BIP39Wordlist))
	...
	bits := new(big.Int)
	for _, w := range words {
		idx, found := wordMap[strings.ToLower(w)]
		if !found {
			return nil, fmt.Errorf("word '%s' is not in the BIP-39 wordlist", w)
		}
		bits.Lsh(bits, 11)
...
// BIP39Wordlist contains the standard 2048 English BIP-39 words.
var BIP39Wordlist = []string{
	...
	"search", "season", "seat", "second", "secret", "section", "security", "seduce", "see", "seed",
