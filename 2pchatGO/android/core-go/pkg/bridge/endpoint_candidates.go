...
func decodeEndpointGroups(raw string) (fresh, reserve []string, err error) {
	if len(raw) > 16384 {
		return nil, nil, errors.New("endpoint request too large")
	}
	...
	if len(fresh)+len(reserve) == 0 || len(fresh)+len(reserve) > 16 {
		return nil, nil, errors.New("invalid endpoint count")
	}
	for _, endpoints := range [][]string{fresh, reserve} {
		for _, ep := range endpoints {
			if ep == "" || len(ep) > 512 || strings.Contains(ep, ",") {
...
func connectEndpointGroups(fresh, reserve []string, connect func(string) error) error {
	...
	if len(reserve) > 0 {
		return connect(strings.Join(reserve, ","))
	}
	return err
}
